...
Groq's hosted whisper-large-v3-turbo via the official groq Python SDK.

Rate limiting
-------------
Groq free tier: 20 requests per minute for Whisper (~1 req / 3 sec).
A monotonic pacer enforces a 3.1-second gap between API calls.
While the plugin sleeps, the audio queue accumulates audio naturally;
`_get_latest_chunk` batches it into one larger chunk automatically.

API Key masking
---------------
The raw API key is XOR-obfuscated with a per-instance random salt in
memory. The clear-text key is only reconstructed for the microsecond
needed to build the Groq client, then the client object is discarded.
"""
...
def _xor_mask(data: bytes, key: bytes) -> bytes:
    """Simple XOR obfuscation — keeps the key out of plain-text memory."""
    return bytes(b ^ key[i % len(key)] for i, b in enumerate(data))


class GroqWhisperProvider(TranscriptionProvider):
    """
    Transcribes audio via Groq's whisper-large-v3-turbo endpoint.
    Uses the official `groq` Python SDK.
    Enforces a 3.1-second pacing interval to stay safely under 20 RPM.
    """
    ...
    def transcribe(self, audio: np.ndarray, **kwargs: Any) -> str:
        ...
        Args:
            audio: numpy float32 array, values in [-1.0, 1.0], 16 kHz mono.
        """
