...
class NLLBLocalProvider(TranslationProvider):
    ...
    torch and transformers are imported lazily to keep server boot instant
    """
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        # num_beams=1 is greedy decoding — ~4x faster than beam search (num_beams=4)
        # with negligible quality loss for conversational live speech.
        # Set num_beams > 1 in config for offline/batch use cases.
        self._num_beams = int(self.config.get("num_beams", 1))
        self.device = "cpu"  # Set dynamically during _load_model
